...
class RTM(object):
    ...
    def _sign(self, params):
        ...
        #Added encoding to utf-8 to ensure unicode tasks are processed
...
def openURL(url, queryArgs=None):
    if queryArgs:
        #added queryArgsUnicode to hadnle unicode params in the url query
        queryArgsUnicode = {}
        for k, v in queryArgs.iteritems():
            queryArgsUnicode[k] = unicode(v).encode('utf-8')
        url = url + '?' + urllib.urlencode(queryArgsUnicode)
        #url = url + '?' + urllib.urlencode(queryArgs)
    #LOG.debug("URL> %s", url)
    proxy = pacProxies.proxyList(SERVICE_URL)
    if proxy.proxies != None:
        return urllib.urlopen(url, proxies=proxy.proxies)
    else:
